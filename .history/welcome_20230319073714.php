...
if (isset($_SESSION['portfolio']) && is_numeric($_SESSION['portfolio']) && $_SESSION['portfolio'] > 100) {
    // Check if the start trade button has been clicked and the CSRF token is valid
    if (isset($_POST['start_trade']) && hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        // Regenerate the session ID to prevent session fixation attacks
        session_regenerate_id(true);
        
        // Set a new session variable to indicate that the trade has been started
        $_SESSION['trade_started'] = true;
        
        // Set a cookie to expire after 30 days
        setcookie('trade_started', 'true', time() + (30 * 24 * 60 * 60), '/', '.example.com', true, true);
    }
    
    // Check if the trade has been started
    if (isset($_SESSION['trade_started']) || isset($_COOKIE['trade_started'])) {
        // Display an activated unclickable button
        echo '<button disabled>Trade started</button>';
    } else {
        // Generate a new CSRF token
        $csrf_token = bin2hex(random_bytes(32));
        $_SESSION['csrf_token'] = $csrf_token;
        
        // Display the start trade button
        echo '<form method="post">';
        echo '<input type="hidden" name="csrf_token" value="' . sanitize_input($csrf_token) . '">';
        echo '<button type="submit" name="start_trade">Start trade</button>';
        echo '</form>';
    }
} else {
    // Display an error message if session variable data is not above 100
    echo '<p>You need to have at least 100 data to start a trade.</p>';
}
?>
