                    ...
                    <div class="relative flex flex-col min-w-0 break-words bg-black rounded-lg mb-6 xl:mb-0 shadow-lg">
                      ...
                      if ($_SESSION['portfolio'] > 100) {
                        if(isset($_POST['start_trade'])) {
                            // The Start Trade button has been clicked, display an activated unclickable button
                            echo '<button id="activated-trade-button" disabled>Activated Trade</button>';
                        } else {
                            // The session variable data is above 100, so display the Start Trade button
                            echo '<form method="POST">';
                            echo '<button id="start-trade-button" type="submit" name="start_trade">Start Trade</button>';
                            echo '</form>';
                        }
                        } else {
                            // The session variable data is not above 100, so don't display the start trade button
                            echo '<p class="text-gradient">You must have at least 100 USDT in you  portfolio to start trading.</p>';
                        }
                        ?>
                        <?php
                          session_start();

                          if(isset($_SESSION['portfolio']) && $_SESSION['portfolio'] > 100){
                              echo '<button>Activated Trading</button>';
                          } else {
                              echo '<button>Inactive Trading</button>';
                          }
                          ?>
