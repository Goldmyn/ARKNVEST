...
if (isset($_POST['activate'])) {
    ...
    $sql = "UPDATE activation_status SET status='$status', end_time='$end_time' WHERE id=1";
...
if (isset($_POST['deactivate'])) {
    ...
    //update status in the database
    $sql = "UPDATE  SET status='$status', end_time=NULL WHERE id=1";
    mysqli_query($link, $sql);
}
...
if (isset($_SESSION['end_time'])) {
    ...
    if ($remaining <= 0) {
        ...
        //update status in the database
        $sql = "UPDATE activation_status SET status='$status', end_time=NULL WHERE id=1";
        mysqli_query($link, $sql);
    }
}
